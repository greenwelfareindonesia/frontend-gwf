  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setNotification("Error creating post. Please try again.");
      }
    } catch (error) {
    ...
    <DashboardSection titleField="Add Post">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-center mt-10">
          <button type="submit" className="py-2 font-semibold text-white rounded bg-primary-2 px-96" disabled={isSubmitting}>
            Post Merch
          </button>
        </div>
        {notification && (
          <div className="mt-4 text-center text-primary-2">
            {notification}
          </div>
        )}
      </form>
