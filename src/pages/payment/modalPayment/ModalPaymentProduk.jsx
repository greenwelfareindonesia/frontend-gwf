...
export default function ModalPaymentProduct({
  ...
  const fetchProducts = async () => {
    try { 
      ...
      if (response.data.data.StatusPayment === "settlement") {
  ...
  useEffect(() => {
    const statusCheckInterval = setInterval(fetchProducts, 5000); // Check every 5 seconds
      ...
      >
        <div className="my-4 text-center w-[100%] m-auto p-5 rounded-xl space-y-4 ">
          <div className="text-left space-y-4">
            ...
            <div className="flex justify-between items-center">
              <div className="space-y-1">
                ...
                {saveRespMidtrans &&
                  saveRespMidtrans.va_numbers &&
                  saveRespMidtrans.va_numbers.map((item, index) => (
                    <div key={index} className="font-semibold">{item?.va_number}</div>
                  ))}
              </div>
              <button
                onClick={() =>
            ...
            <button className="p-2 bg-green-600 hover:bg-green-800 w-full rounded-2xl text-white font-bold" onClick={fetchProducts} type="button">
              Check Status
            </button>
          </div>
        </div>
